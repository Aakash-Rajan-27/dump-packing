        ...
        if self.status in (self.STATUS_IDLE, self.STATUS_WAITING):  # nothing to do this tick
            ...
            if not self.path:  # path just ran out — check whether we reached the intended stop
                ...
                if stop_cell and (tr, tc) == stop_cell:  # did we land exactly on the stop cell?
                    ...
                    _fc = self.front_center_cell(grid)
                    _fc_z  = grid.z_height[_fc[0], _fc[1]]
                    _fc_st = grid.state[_fc[0], _fc[1]]
                    print(f"[NAV-FAIL] T{self.id} path exhausted without reaching stop. "
                          f"front={_fc} z={_fc_z:.2f} st={_fc_st} "
                          f"expected_stop={stop_cell} dump_target={self.dump_target}")
                    if self.dump_target:
            ...
            if is_pose_driveable(grid, self, next_x, next_y, self.heading):
                ...
                if self._reverse_stuck == 1:
                    _br, _bc = grid.world_to_cell(next_x, next_y)
                    print(f"[REVERSE-BLOCKED] T{self.id} reverse step blocked at "
                          f"({_br},{_bc}) z={grid.z_height[_br,_bc]:.2f} "
                          f"state={grid.state[_br,_bc]}")
                if self._reverse_stuck >= 5:
                    print(f"[REVERSE-STUCK] T{self.id} giving up reverse after 5 steps — dumping in place")
                    self._reverse_stuck = 0
                    self.status      = self.STATUS_DUMPING
                    self._dump_ticks = 0
