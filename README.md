...
*Source: `spacing_display/` folder, `origin/Ritvik` branch, commit `87a07e0` ("Use for main final ppt"). See §8 for why this specific folder/branch.*
...
![The simulator running on a freshly generated, irregular paddock polygon — no pre-defined spot points or lanes, just the live grid and the entry gate](assets/world_random_polygon.png)
*A randomly generated dump-site polygon at tick 0 — this is an actual run of the codebase (`random_shaped_algo/polygon_gen.py`), not a mockup. Irregular, non-rectangular boundaries like this are exactly the case pre-defined spot-point grids handle badly (§1).*
...
2. **Irregular polygons leave dead zones.** A regular spot-point grid never tiles an irregular paddock boundary exactly — the corners are always wasted.
...
Material is deposited across every cell inside that radius, then an iterative relaxation pass caps the slope between any two adjacent cells at `tan(θ)` — the same way real debris piles self-level. This matters for the whole pipeline downstream: packing density, accessibility, and even where the *next* truck can physically stand are all read off a terrain model that behaves like real dumped material, not an idealized grid of on/off cells. It's also what makes "spot points must be defensively over-spaced" (the failure mode this project is designed around) directly testable — the simulator can't cheat by ignoring realistic pile geometry.
...
|---|---|
| ![Debris cone gradient rendered on the fine grid, with a truck for scale](assets/debris_cone_thumbnail.png) | ![Live screenshot of overlapping debris piles and a truck-width path corridor cutting through the site](assets/debris_piles_live.png) |
...
The second check is the expensive one — naively it's a full BFS over the grid for every candidate cell. The key theoretical shortcut used here: **isolation is a topological property**, so it can be tested on the coarsened grid without losing correctness, as long as the coarsening factor is smaller than the smallest obstacle the check needs to detect. This turns an O(cells) BFS per candidate into an O(cells / block²) one — a large constant-factor speedup with no accuracy loss for this specific question. A cheap local pre-filter (is this cell "pinched" by two or more blocked neighbours at all?) additionally skips the BFS outright for the common case where a cell obviously isn't a chokepoint.

> 🖼️ **Add a picture here** — a before/after pair showing a candidate dump that *would* isolate a pocket (rejected) vs. one that doesn't, or an overlay of the driveable mask at one heading. Suggested filename: `assets/accessibility_check.png`. This stage doesn't have a natural renderer view yet since it's an internal filter — a debug overlay (even a quick matplotlib plot of the BFS-reachable set) would work well.
...
| Score surface after one dump | Score surface after two dumps |
|---|---|
| ![Heatmap of the combined heuristic score across the site after a single dump, brightest near the pile and near the boundary](assets/scoring_heatmap_single.png) | ![Heatmap after a second dump showing a second bright region forming](assets/scoring_heatmap_dual.png) |

*These are the team's own visualisations of the backend scoring calculation (not the game renderer) — brighter = higher combined score. Boundary cells score higher, and density scoring pulls the peak toward existing piles.*

### 2.4 MCTS lookahead: correcting for the greedy heuristic's myopia
...
**Monte Carlo Tree Search** corrects for this by simulating a few dumps ahead before committing to the first one. Each node in the search tree is a candidate cell; **UCB1** governs which node to expand next, balancing exploitation of cells that have scored well so far against exploration of cells that haven't been tried much yet:
...
> 🖼️ **Add a picture here** — ideally a short GIF/video of the MCTS rollout tree growing (nodes lighting up as they're visited), or a before/after pair: the raw heuristic's top pick vs. the pick MCTS actually recommends once a bad long-term choice is penalised. Suggested filename: `assets/mcts_lookahead.gif`.
...
![Bipartite matching diagram: four trucks, four dump points, all possible pairings shown faint, the optimal (minimum-total-cost) matching highlighted](assets/hungarian_assignment.png)
...
![The core challenge: a path to a point (x,y) via ordinary A* is a different, easier problem than a path that must arrive at a specific pose (x,y,θ) — solved here with hybrid A*](assets/staging_hybrid_astar_concept.png)
...
|---|---|
| ![Live screenshot of a truck in the REVERSING state, staged just outside its dump target with the clearance circle visible](assets/staging_reversing_live.png) | ![Screenshot showing a truck's purple planned-path corridor curving around and doubling back so the truck can reverse into its dump target](assets/maneuvering_corridor.jpg) |
...
![Live screenshot: three trucks of different classes (small/medium/large) all simultaneously in NAVIGATING status, each with its own path corridor, moving through a partially-filled site](assets/multi_agent_mixed_fleet.jpg)
...
![Live screenshot: a single truck's CBS-planned path (purple corridor) threading between multiple existing debris piles](assets/cbs_corridor_screenshot.png)
...
> 🖼️🎥 **Add media here** — this is the single most useful thing to demonstrate live, since it's the area actively being hardened: a short video/GIF of two trucks approaching head-on, one performing the reverse-retreat + yield maneuver, and both continuing past each other. Suggested filename: `assets/deadlock_yield_demo.gif`.
...
| **The dump site itself** | `POLYGON_BOUNDARY` (list of `(x, y)` vertices) | Any simple polygon works. `random_shaped_algo/polygon_gen.py` (see §7) can generate one for you — pass `--seed N` to `main.py` there to reproduce a specific irregular shape, as in the hero image at the top of this document. |
...
## 4. Tech stack & why these tools

| Piece | Library | Why |
|---|---|---|
| Grid & sandpile physics | **NumPy** | The whole grid (tens of thousands of cells) is stored as N-dimensional arrays; height checks, slope relaxation, and driveable-mask construction are vectorized matrix operations, not per-cell Python loops — this is what keeps a full tick's terrain update cheap enough for real-time rendering. |
| Boundary / footprint checks | **Shapely** | Point-in-polygon and oriented-footprint checks, so a truck (or a hypothetical dump) is never accepted if it clips the paddock boundary. |
| Truck ↔ dump-point matching | **SciPy** (`linear_sum_assignment`) | The Hungarian algorithm implementation backing §2.5. |
| Dump/pheromone smoothing | **SciPy** (`ndimage`) | Gaussian/uniform filtering for pheromone diffusion and boundary erosion. |
| MCTS search tree | Plain Python tree nodes | Each node holds a hypothetical future grid state; visited/value counters implement UCB1 (§2.4). |
| CBS / hybrid-A* priority queues | **heapq** | Min-heaps for best-first expansion in both the CBS constraint tree and the underlying A* variants — O(log n) push/pop. |
| Driveable state | 3D NumPy tensor `(row, col, heading)` | Rather than a single 2D "is this cell free" map, driveability is heading-dependent (a cell might be fine to enter facing one way and not another) — the 3D tensor makes that an O(1) lookup instead of a recomputation. |
| Rendering | **Pygame** | Chosen over Matplotlib (no real-time redraw) and RViz (heavy, ROS-coupled) for a real-time, dependency-light visualiser decoupled from the core simulation logic. |
| Concurrency | Python **threading** | See §2.9 / §7 — planning runs on a background thread against snapshots so it never blocks truck movement or rendering. |

## 5. Results

The team's own validation runs on the reference hexagonal polygon reached:

- **Final packing density > 60%**
- **Achieved dump spacing < 5 m** (against the ~7.4 m autonomous baseline and ~3.0 m staffed-operator benchmark from the problem brief)

![Final state of a packed site — piles spread fairly evenly across the polygon, well short of the 3m staffed benchmark's density but comfortably inside the target band](assets/results_final_spread.png)

These numbers come from the team's own demo runs rather than a controlled multi-trial benchmark (MCTS rollouts and initial terrain seeding are randomized, so a single run's number has some spread) — see the companion metrics-plan discussion for how to turn this into a rigorous, repeatable benchmark (multiple seeded trials, a staggered-grid baseline for comparison, and a scalability sweep across truck count).

## 6. Metrics tracked live
...
![Development view: the renderer and the terminal's live Hungarian cost-matrix output side by side during an actual run](assets/dev_view_terminal_and_sim.jpg)
...
| `random_shaped_algo/polygon_gen.py` | *(separate branch folder, see §9)* Procedural irregular-polygon generator used for the hero screenshot and the robustness testing described in §1. |
...
The GitHub repo (`github.com/Aakash-Rajan-27/dump-packing`) has diverged across `master` and three personal branches (`aakashr`, `Ritvik`, `aditya`). This document covers `spacing_display/` on `origin/Ritvik`, commit `87a07e0` — the only commit in the repo explicitly labelled as the final/competition version, and the most complete implementation of the pipeline above. `master` still points at an earlier 10-module version; consolidating onto one branch/folder (and adding a `requirements.txt`) is recommended before submission so there's no ambiguity about which code is "the" entry. `random_shaped_algo/` (also on `origin/Ritvik`) is a near-identical copy of `spacing_display/` with a procedural polygon generator swapped in — used above for the irregular-site demonstration.

## 10. Media checklist — everywhere a picture or video is still needed

All of the images below are already in `assets/` and referenced inline above — pulled either from the team's PPT or captured from an actual run of this codebase during this pass. What's still missing:

| Location | What to capture | Suggested filename |
|---|---|---|
| §2.2 Candidate generation & accessibility | A visual of a rejected (isolating) candidate vs. an accepted one, or a driveable-mask overlay | `assets/accessibility_check.png` |
| §2.4 MCTS lookahead | GIF/video of the rollout tree expanding, or a before/after of the greedy pick vs. the MCTS-corrected pick | `assets/mcts_lookahead.gif` |
| §2.8 Deadlock & liveness | Video of the reverse-retreat + yield maneuver resolving a head-on encounter — the most demo-worthy clip given this is the active development area | `assets/deadlock_yield_demo.gif` |
| General | A full end-to-end timelapse (empty site → fully packed), sped up | `assets/full_run_timelapse.gif` |

**On video specifically**: GitHub renders both MP4 (via drag-and-drop into a README edit box, which auto-hosts it and gives you a URL to paste) and GIFs inline. GIFs are more portable if this README is viewed outside GitHub (e.g. pasted into a slide deck or a judge's PDF export) — for a screen recording, `ffmpeg -i recording.mp4 -vf "fps=12,scale=800:-1" assets/name.gif` gives a reasonably small, README-friendly GIF.
