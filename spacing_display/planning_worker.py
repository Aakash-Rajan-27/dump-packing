...
def _execute_planning_task(task, result_q):
    ...
    if ttype == 'idle':
        ...
        for t_snap, orig_dp in assignments_all:
            ...
            _tried = 0
            for fb_dp in top_cands:
                if fb_dp == orig_dp or fb_dp in _claimed_dps:
                    continue
                _tried += 1
                if _tried > _MAX_FB:
                    break
                fb_paths, fb_stg = plan_staging_paths(
                    grid_snap, [(t_snap, fb_dp)], locked_paths=locked,
                    extra_st_constraints=extra_st,
                    precomputed_masks={(repr_truck.truck_class, False): _fine})
                fb_path = fb_paths.get(t_snap.id, [])
                if fb_path:
                    out[t_snap.id] = (fb_path, fb_dp, fb_stg.get(t_snap.id))
                    _claimed_dps.add(fb_dp)
                    print(f"[FALLBACK] T{t_snap.id}: fallback #{_tried} succeeded → {fb_dp}")
                    break
            else:
                print(f"[FALLBACK] T{t_snap.id}: all {_tried} fallback candidates failed")
    ...
    # ── GATE PRE-PLANNING ─────────────────────────────────────────────────────
        ...
        if not path:
            ...
            _tried = 0
            for fb_dp in top:
                if fb_dp == dt or fb_dp in claimed:
                    continue
                _tried += 1
                if _tried > 8:
                    break
                fb_asgn = assign([t_snap], [fb_dp], grid_snap)
                if not fb_asgn:
                    continue
                fb_paths, fb_stg = plan_staging_paths(
                    grid_snap, fb_asgn, locked_paths=locked,
                    ignore_path_reserved=True,
                    precomputed_masks={(t_snap.truck_class, True): _fine})
                fb_path = fb_paths.get(t_snap.id, [])
                if fb_path:
                    dt   = fb_asgn[0][1]
                    path = fb_path
                    sp   = fb_stg.get(t_snap.id)
                    print(f"[FALLBACK] Gate T{t_snap.id}: fallback #{_tried} succeeded → {dt}")
                    break
            else:
                print(f"[FALLBACK] Gate T{t_snap.id}: all {_tried} fallback candidates failed")
