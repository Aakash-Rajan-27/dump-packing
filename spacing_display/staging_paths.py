...
import math
import grid_map
from path_utils import _path_cells, _dedup_path_cells, _truck_front_cell, _corridor_cell_set
...
def plan_staging_paths(grid, assignments, locked_paths=None, max_time=ASTAR_MAX_TIME,
    ...
    if locked_paths:
        for path_entry in locked_paths.values():
            ...
            # Deduplicate so hybrid-A* locked paths share the time axis with
            # hybrid_astar_to_staging_st cell steps (both ~1 unique cell/step).
    ...
    if extra_st_constraints:
        locked_st |= extra_st_constraints
    ...
    if not locked_st:
        ...
        if len(_sp_paths) == len(agent_info):
            ...
            if _detect_physical_conflict(_sp_cells, _sp_truck_map, grid) is None:
