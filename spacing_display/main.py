...
def run_simulation():
    ...
    # ── Dump-centre spacing tracking ──────────────────────────────────────────
    # Captured at assignment time (when set_path / preload_dump_path is called).
    # Running average: each new point does one O(n) scan for its nearest
    # existing centre, adds that distance to _nn_sum, increments _nn_count.
    # avg = _nn_sum / _nn_count  (never recomputed from scratch).
    _dump_centres: list  = []   # [(wx, wy), ...] in assignment order
    _seen_dp:      set   = set()  # (r,c) cells already recorded — prevents double-counting on replan
    _nn_sum:       float = 0.0
    _nn_count:     int   = 0
    _avg_spacing:  float = 0.0
    ...
    while not done:
        ...
        while not _result_q.empty():
            ...
            if res['type'] == 'idle':
                ...
                for tid, (path, dp, sp) in res['assignments'].items():
                    ...
                    # Record dump centre the moment it is assigned.
                    if dp and dp not in _seen_dp:
                        _seen_dp.add(dp)
                        _wx, _wy = grid.cell_to_world(*dp)
                        if _dump_centres:
                            _nn_d = min(math.hypot(_wx - cx, _wy - cy)
                                        for cx, cy in _dump_centres)
                            _nn_sum   += _nn_d
                            _nn_count += 1
                            _avg_spacing = _nn_sum / _nn_count
                        _dump_centres.append((_wx, _wy))
                        print(f"[SPACING] assigned dp={dp} n={len(_dump_centres)} "
                              f"avg_nn={_avg_spacing:.2f}m")
                ...
                if dp and dp not in _seen_dp:
                    ...
                    if _dump_centres:
                        ...
                        _avg_spacing = _nn_sum / _nn_count
                    _dump_centres.append((_wx, _wy))
                    print(f"[SPACING] gate dp={dp} n={len(_dump_centres)} "
                          f"avg_nn={_avg_spacing:.2f}m")
        ...
        for substep in range(STEPS_PER_TICK):
            ...
            renderer.draw(trucks, metrics, avg_spacing=_avg_spacing)
