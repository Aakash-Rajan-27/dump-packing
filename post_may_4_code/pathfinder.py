...
from config import (DRIVE_CLEARANCE_M, _TAN_REPOSE, ENTRY_POINT,
                    ...
                    ENTRY_CORRIDOR_CELLS, ASTAR_MAX_TIME, CBS_MAX_NODES)
...
def plan_staging_paths(grid, assignments, locked_paths=None, max_time=ASTAR_MAX_TIME,
    ...
    if len(_sp_paths) == len(agent_info):
        ...
        # Use cell-level conflict only (truck_map=None disables SAT).
...
def plan_paths_cbs(grid, assignments, locked_paths=None):
    ...
    # ── SINGLE-AGENT SHORT-CIRCUIT ────────────────────────────────────────────────
    # One truck: no inter-agent conflict possible — use fast spatial astar directly.
    if len(agent_info) == 1:
        aid  = next(iter(agent_info))
        info = agent_info[aid]
        ...
        if not path:
            # spatial astar failed (truck may be boxed in) — try ST with locked constraints
    ...
    # ── SPATIAL-FIRST (APPROACH B) ────────────────────────────────────────────────
    # Try fast spatial A* for all agents first.
    # Only fall back to expensive ST A*/CBS if spatial paths actually conflict.
    _sp_paths = {}
    ...
    # Cell-level conflict only (truck_map=None disables SAT — see staging equivalent above).
    if _detect_first_conflict(_cell_sp, truck_map=None, grid=None) is None:
        return smooth_paths(_sp_paths)   # no conflict — skip ST/CBS entirely
    # ─────────────────────────────────────────────────────────────────────────────
