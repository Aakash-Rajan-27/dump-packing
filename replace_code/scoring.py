...
from config import WEIGHTS_EARLY, WEIGHTS_MID, WEIGHTS_LATE, TARGET_PILE_HEIGHT, ENTRY_POINT
...
def score_candidates(grid, candidate_idxs):
    ...
    reserved_mask   = (grid.state == CellState.RESERVED)
    ...
    occupied_mask = filled_mask | reserved_mask
    zone_fill    = uniform_filter(occupied_mask.astype(float), size=8)
    ...
    # ── Score 4: Pheromone ──────────────────────────────────
    # Avoid cells with recent truck presence:
    # pheromone 1.0 = recently used (bad), 0.0 = untouched (good)
    phero_score = 1.0 - grid.pheromone
    ...
    # Score 6: Entry distance — prefer cells far from entry point
    entry_world_x, entry_world_y = ENTRY_POINT
    entry_row = (entry_world_y - grid.origin[1]) / grid.cell_size
    entry_col = (entry_world_x - grid.origin[0]) / grid.cell_size
    dist_to_entry = np.sqrt(
        (row_idx - entry_row) ** 2 +
        (col_idx - entry_col) ** 2
    )
    max_dist = np.sqrt(grid.rows ** 2 + grid.cols ** 2)
    entry_dist_score = dist_to_entry / (max_dist + 1e-6)
