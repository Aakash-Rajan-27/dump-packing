...
import math
import grid_map
from path_utils import _path_cells, _dedup_path_cells, _truck_front_cell, _state_cell
from bicycle_model import interpolate_path_to_truck_states
from astar_core import astar, astar_st
from conflict_detect import _detect_first_conflict, _detect_physical_conflict
...
def plan_paths_cbs(grid, assignments, locked_paths=None, extra_st_constraints=None):
    """
    ...
    if locked_paths:
        for path_entry in locked_paths.values():
            ...
            # Deduplicate consecutive same cells so hybrid-A* locked paths (4
            # waypoints/m) share the same time axis as astar paths (1 cell/step).
            cells = _dedup_path_cells(grid, raw_path)
    ...
    if extra_st_constraints:
        locked_st_constraints |= extra_st_constraints
    ...
    _sp_truck_map = {aid: agent_info[aid]['truck'] for aid in agent_info}
    _sp_clear = not locked_st_constraints
    if _sp_clear and _detect_physical_conflict(_cell_sp, _sp_truck_map, grid) is None:
    ...
    if not _sp_clear:
        ...
        for _aid, _cells in _cell_sp.items():
            ...
            if _locked_hit:
                break
        if not _locked_hit and _detect_physical_conflict(_cell_sp, _sp_truck_map, grid) is None:
            return smooth_paths(_sp_paths)
    # ─────────────────────────────────────────────────────────────────────────────
