...
def make_driveable_mask(grid, truck, ignore_path_reserved=False):
    ...
    # Cells with <= 0.5 m of material are always driveable even if PATH_RESERVED.
    # Trucks physically pass through each other, so low-pile reservations must not block routes.
    _low_z = (
        ~near_boundary
        & (grid.z_height <= 0.5)
        & ~np.isin(grid.state, [CellState.BOUNDARY, CellState.FILLED, CellState.OBSTACLE])
    )
    base_ok = _low_z | (
        (~near_boundary) & (~np.isin(grid.state, blocked_for_base)) & (grid.z_height <= max_escape_height)
    )
    ...
    for hi in range(POSE_HEADING_BUCKETS):
        ...
        for ci in range(4):
            ...
            # Low-pile corners (<= 0.5 m) are passable even if PATH_RESERVED
            _low_z_c = (corner_z <= 0.5) & ~np.isin(corner_state, [CellState.FILLED, CellState.OBSTACLE, CellState.BOUNDARY])
            is_blocked &= ~_low_z_c
...
def is_pose_driveable(grid, truck, x, y, heading, margin_m=None, ignore_path_reserved=False):
    ...
    for ls in (-1, 1):
        for ws in (-1, 1):
            ...
            z  = grid.z_height[r, c]
            st = grid.state[r, c]
            # Low-pile cells (<= 0.5 m) are always passable even if PATH_RESERVED
            if z <= 0.5 and st not in (CellState.FILLED, CellState.OBSTACLE, CellState.BOUNDARY):
                continue
            if st in blocked:
                return False
            if z > DRIVE_CLEARANCE_M + 0.5:
                return False
    return True
