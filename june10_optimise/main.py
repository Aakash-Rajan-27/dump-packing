...
#
# FIX: Stationary Truck Locks (DUMPING, REVERSING, IDLE, STUCK)
#      are now properly passed to the space-time pathfinder.
# FIX: Added KeyboardInterrupt hook to print polygon coordinates.
# ─────────────────────────────────────────────────────────────
...
def _try_inplace_replan(ta, tb, grid, entry_rc):
    ...
    if exit_t._exit_path and nav_t.dump_target:
        ...
        np_res, ns = plan_staging_paths(grid, [(nav_t, nav_t.dump_target)], locked_paths=locked)
        np_nav = np_res.get(nav_t.id, [])
        if np_nav:
    ...
    while not done:
                            ...
                            if _dpts:
                                ...
                                if _asgn:
                                    ...
                                    for _ot in trucks:
                                        ...
                                        # FIX 1: Treat parked trucks as solid walls
                                        elif _ot.status in (_ot.STATUS_DUMPING, _ot.STATUS_REVERSING, _ot.STATUS_STUCK, _ot.STATUS_IDLE):
                                            _locked[_ot.id] = _locked_path_for(_ot, [])
        ...
        if exiting_trucks:
            ...
            for t in trucks:
                ...
                # FIX 2: Treat parked trucks as solid walls
                elif t.status in (t.STATUS_DUMPING, t.STATUS_REVERSING, t.STATUS_STUCK, t.STATUS_IDLE):
                    locked_moving[t.id] = _locked_path_for(t, [])
            ...
            if top_candidates:
                ...
                if assignments_all:
                    ...
                    for t in trucks:
                        ...
                        # FIX 3: Treat parked trucks as solid walls
                        elif t.status in (t.STATUS_DUMPING, t.STATUS_REVERSING, t.STATUS_STUCK, t.STATUS_IDLE):
                            all_locked[t.id] = _locked_path_for(t, [])
...
if __name__ == '__main__':
    try:
        ...
        # ─────────────────────────────────────────────────────────────
        # POLYGON CAPTURE FEATURE
        # If you stop the simulation with Ctrl+C, it prints the exact
        # coordinates to the terminal so you can easily reuse them!
        # ─────────────────────────────────────────────────────────────
        from config import POLYGON_BOUNDARY
        print("\n" + "="*60)
        print("SIMULATION INTERRUPTED (KeyboardInterrupt)")
        print("To reuse this exact map boundary in the future, copy these coordinates:")
        print("POLYGON_BOUNDARY = [")
